...
echo   '3. Creating the channel: channel - may fail if already created'
peer channel create -o $ORDERER_ADDRESS -c channel -f $CONFIG_DIRECTORY/channel.tx

# If channel already created then -  use the following to fetch channel genesis
#peer channel fetch 0 -o localhost:7051  -c channel  channel.block
...
peer channel join -o $ORDERER_ADDRESS -b ./channel.block
